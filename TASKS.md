...
- [ ] **P18.2** — Add tool-calling throughput benchmark (deferred from P17.3). Requires a model that reliably chooses to call the tool; without careful prompt engineering the run is dominated by model decision time. A per-provider prompt template + a separate scenario file under `apps/cli/test/perf/` is the cleanest split.
- [ ] **P18.3** — Concurrent agent runs / load test. Out of P17 scope; needs coordinated cancellation, per-tenant rate-limit accounting, and a more sophisticated harness. Worthwhile once the perf baseline from P17.3 is captured.
