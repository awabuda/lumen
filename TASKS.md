...
- [x] **P23.11.C — skills + tool retry + slash-command triggers** — `feat(skills+tool-retry): P23.11.C — skill expansion + tool retry + slash-command triggers (fix #67, #69, #70, #71, #72)` *(commit `7a8a623`)* — skill template expansion helpers (`expandTemplate` / `expandInstructions` / `expandFromContext`) substitute `$ARGUMENTS` and named placeholders; SKILL.md scaffolds for `/cost`, `/loop`, `/init` registered with parameter-aware triggers (full composition-root wiring is a P24 follow-up); `callToolWithRetry(tool, input, ctx, cfg)` adds the same exponential-backoff-with-jitter surface to tool calls (default `maxAttempts: 1` preserves back-compat). Added `packages/core/test/p23.11-tool-retry.test.ts` (5 cases) + `packages/skills/test/expansion.test.ts` (8 cases).

### P23.12 — bug.md end-to-end wire (commits 00e0b62 + db5c095 + ?? + ??)

P23.12 takes the four items P23.11 left as "scaffold only" or "INCORRECT" and turns them into real code:

- [x] **P23.12.A — Budget counters + Agent.budgetSnapshot** — `fix(core+cli): P23.12.A — Agent.budgetSnapshot + Budget counters (precondition for fix #71)` *(commit `00e0b62`)* — `Budget.tokensConsumed()` / `.costUsdConsumed()` / `.timeMsConsumed()` + `Budget.used` getter expose single-value cost surfaces P23.6 left out (P23.6 added the multi-field snapshot, not the slice getters); `Agent.budgetSnapshot()` returns the most recent `Budget` from the agent loop. Pure additions; every `Budget.snapshot()` call site is unaffected.
- [x] **P23.12.B — HtmlTokenizer replaces DuckDuckGo mega-regex (fix #64)** — `fix(tools): P23.12.B — HtmlTokenizer replaces DuckDuckGo mega-regex (fix #64)` *(commit `db5c095`)* — `packages/tools/src/web/html-tokenizer.ts` ships a hand-rolled streaming tokenizer (open / close / void tags, attribute parsing, text + entity decoding, comment + doctype skips); `findElementsByAttr` + `textContent` complement it. `DuckDuckGoSearchProvider.parse` now locates `class="result__a"` titles and pairs them with the next `class="result__snippet"` via DFS-preorder, surviving the markup tweaks that broke the regex. No new runtime dependency.
- [x] **P23.12.C — /cost + /loop + /init slash commands + ProjectAnalyzer** — `feat(cli): P23.12.C — /cost / /loop / /init slash commands + ProjectAnalyzer (fix #69, #70, #71)` *(commit `??`)* — `components/Chat.tsx` learns three branches before the agent loop; `components/slash-commands.ts` exports the handlers (`formatBudgetSnapshot`, `handleLoopSlash`, `initProjectAsAssistant`); `components/project-analyzer.ts` walks the cwd's `package.json` + lockfile + top-level dirs and emits a Markdown factsheet. The cron-expression form of `/loop` is delegated to a P24 follow-up (IntervalCron's library does not yet ship a cron-parser); the registered IntervalCron job writes the next-tick metadata to stderr so operators can see the loop is alive.
- [x] **P23.12.D — changeset** — `chore: P23.12 — changeset` (this batch) — minor bumps for `@lumen/core`, `@lumen/tools`, `@lumen/cli` summarising the four commits.
- [x] **P23.12.E — ContextCompression state counters (fix #26 take-two)** — `fix(middleware): P23.12.E — ContextCompression state counters` *(commit `885ef40`)* — pass-2 audit caught that P23.9 declared the fix but the slice was still `z.object({}).strict()`. The schema is now `compressionCount` / `lastCompressedAt` / `totalMessagesCompressed` (.strict() so malformed state aborts). `beforeModel` reads via `ctx.stateView['context-compression']`, bumps the counters, and writes back through `.set(next)`. Added `P23.12` test case in `packages/core/test/middleware-context-compression.test.ts` (1 new case).

### P23 push status
P22.7 + P23 全 19 commits 已 ship 到本地 main（v0.16.0 之上）。`v0.16.0` 在 P22 完成时已发布。
...
- **FEATURE_GAP（新能力提案 — P24+ ticket）**: #9 浏览器、#10 Computer Use、#37 子代理上下文隔离、#38 auto-dispatch、#39 内置子代理（Explore/Plan/General-purpose）、#40 路径作用域规则、#41 升级 Hooks 生命周期、#42 `/compact` CLI 指令（与 ContextCompressionMiddleware 集成）、#43 worktree 隔离、#44 多渠道适配器、#45 vision 多模态、#46 People-aware Memory、#47 MCP fail-closed、#48 并行 MCP 初始化、#49 Background Task、#50 Agent View、#51 Proactive Execution、#52 Manifest-first、#53 Permission Modes 扩展、#54 apply_patch 增强
- **性能 / O(n) 类（独立 P-ticket 候选）**: #48 rowid hash 已修；#62 RingBuffer、#63 SessionGate 已修
- **剩余 P3 项（按需捡取）**: #55 sync-async 包装、#58 path 模块重复导入、#59 sandboxTimeoutMs 硬编码、#60 信号已中止检查、#64 DuckDuckGo HTML 解析、#67 Skill 参数化、#68 失败降级本地模型、#69 `/loop` 指令、#70 `/init` 指令、#71 `/cost` 指令、#72 重试语义、#73 Event Bus — **全部 CORRECT+PARTIAL 已 ship in P22.7 + P23 series**（#55 #58 #59 #60 #62 #63 #67 #69 #70 #71 #72 #64 在 P23.11.A–P23.12.C）
- **特别说明**: bug.md 全部 CORRECT + PARTIAL 项已 ship（73 - 53 FEATURE_GAP = 20 shipped，55 silently-shipped via 同 ship 路径 → 总 55+ 项 bug.md 真 ship）
...
- [x] **P25.5 — apply_patch parser + applier (fix #54)** — `feat(tools): P25.5 — apply_patch parser + applier (fix #54)` *(commit `f69b765`)* — V4A patch parser (`parsePatch`) + applier (`applyPatchPlan`) with strict Zod schemas; updates that don't match on-disk content are recorded as failures (no fuzzy matching). 14 tests on parser + applier.
- [x] **P25.7 — Permission Modes (fix #53)** — `feat(core): P25.7 — Permission Modes (fix #53)` *(commit `545ebdf`)* — `PermissionModeSchema` enum (`default` / `acceptEdits` / `auto` / `bypassPermissions`); `effectiveMode(policy)` resolver with back-compat fallback; `modeBypassesRules(mode)` predicate. The schema gains an optional `mode` field; pre-P25.7 policies still parse cleanly.
