 ...
 * determined.
 */
function replaceOneNormalized(original, needle, normIdx, replacement) {
    ...
    for (let i = 0; i <= original.length; i++) {
        ...
        if (/\s/.test(ch)) {
            ...
            if (!/\s/.test(prev))
                nIdx++; // opening of a new whitespace run
        }
        else {
            nIdx++;
        }
    ...
    if (origStart < 0)
        throw new Error('normalized index out of range');
    ...
    for (; end < original.length && advanced < needle.length; end++) {
        ...
        if (/\s/.test(ch)) {
            const prev = end > 0 ? original[end - 1] : '';
            if (!/\s/.test(prev))
                advanced++;
        }
        else {
...
export class PatchTool extends BaseTool {
    name = 'patch';
    description = 'Find-and-replace text inside a file. oldString must match exactly once unless replaceAll is set. ' +
    ...
    async execute(input, ctx) {
        const { path: userPath, oldString, newString, replaceAll } = input;
        ...
        if (exactCount > 0) {
            if (exactCount > 1 && !replaceAll) {
                throw new Error(`patch: oldString matched ${exactCount} times in ${absPath} but replaceAll=false. ` +
        ...
        if (normCount > 1 && !replaceAll) {
            throw new Error(`patch: oldString matched ${normCount} times under fuzzy whitespace normalization in ${absPath} ` +
