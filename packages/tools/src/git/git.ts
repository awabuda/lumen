 ...
 * the descriptor is the **maximum** risk across all operations;
 * the per-operation risk is checked at runtime so the approval
 * flow can do "git commit" specifically, not "any dangerous op".
 */
type GitOpRisk = 'safe' | 'approval-required'
...
export class GitTool extends BaseTool {
  ...
  public readonly description =
    'Run a whitelisted git operation. Operations: status, diff, log, branch, commit. ' +
  ...
  public readonly inputSchema: z.ZodType<unknown> = GitInputSchema
   ...
   */
  private argvFor(input: GitInput): string[] {
    switch (input.op) {
      case 'status':
