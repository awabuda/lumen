...
export const doctorCommand = async (opts = {}) => {
    ...
    try {
        const { createShellTools, defaultShellSandboxConfig, resolveSandbox } = await import('@lumen/tools');
        ...
        if (outcome.kind === 'ok' && outcome.result.exitCode === 0) {
            ...
            fail(`Shell sandbox round-trip failed: exit=${outcome.kind === 'ok' ? outcome.result.exitCode : 'n/a'}`);
        }
    }
    catch (err) {
