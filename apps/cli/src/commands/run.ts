...
export const runCommand = async (options: RunCommandOptions): Promise<number> => {
  ...
  // composition.ts now throws `ConfigError` instead of falling
  // back to `gpt-4o-mini` (the OpenAI-only id that failed
  // silently on every other OpenAI-compatible endpoint). We
  // print a one-line hint HERE so the operator learns about
  // the missing config before composition.ts throws.
  const hasModel =
  ...
  if (!hasModel) {
    ...
    try {
      ...
      resolvedFromConfig = cfg.defaultModel !== undefined
    } catch {
